...
export default function Historico() {
  ...
  const performanceStats = summary ? [
    { label: 'Total de Bets', value: totalItems.toLocaleString(), icon: Activity, color: 'text-indigo-600', bg: 'bg-indigo-50' },
    { label: 'Win Rate', value: `${winRate}%`, icon: Target, color: 'text-emerald-700', bg: 'bg-emerald-50' },
    { label: 'ROI Geral', value: `${summary.overallRoi}%`, icon: BarChart, color: 'text-emerald-700', bg: 'bg-emerald-50' },
    { label: 'Lucro Total', value: `R$ ${summary.monthlyProfit.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`, icon: Trophy, color: 'text-amber-500', bg: 'bg-amber-50' },
  ] : [];
          ...
          <div className="px-6 py-4 bg-slate-50/50 border-t border-slate-800 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-3 py-1 bg-white border border-slate-800 rounded text-[10px] font-black uppercase text-slate-600 hover:bg-slate-50 transition-colors shadow-sm disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-1"
              >
